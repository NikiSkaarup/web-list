<script lang="ts">
	...
	import { torrents, categories, tags } from '$lib/stores/maindata.store';

	const splitTags = (torrent: QbtTorrent | undefined) => {
		if (torrent?.tags === undefined || torrent.tags.length === 0) return [];
		return torrent.tags.split(',').map((tag) => tag.trim());
	};
	...
	const tagsAutoComplete: AutocompleteOption[] = $tags.map((tag) => ({
		value: tag,
		label: tag
	}));
	...
	function onTagSelect(event: any) {
		if (existingTags.includes(event.detail.value)) return;
		existingTags.push(event.detail.value);
		existingTags = existingTags;
		tagsInput = '';
	}
</script>

{#if torrent !== undefined && fields !== undefined}
	<div class="space-y-4 h-full">
		...
		>
			<legend class="legend px-4">
				<p
					class="text-sm text-surface-800-100-token max-w-full text-ellipsis overflow-hidden"
					title={torrent.name}
				>
					{torrent.name}
				</p>
				<p
					class="text-sm text-surface-700-200-token max-w-full text-ellipsis overflow-hidden"
					title={torrent.hash}
				>
					{torrent.hash}
				</p>
			...
			<div class="flex-grow flex flex-col space-y-4 overflow-y-auto px-4">
				...
				<div
					class="card flex w-full max-h-32 min-h-[7rem] p-4 overflow-y-auto"
					tabindex="-1"
				>
					<Autocomplete
						bind:input={tagsInput}
						options={tagsAutoComplete}
						denylist={existingTags}
